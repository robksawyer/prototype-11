...
describe('components', () => {
  describe('StatsGroup', () => {
    it('should mount with props', function () {
      const wrap = mount(<StatsGroup>Hello World</StatsGroup>)
    ...
    it('should render as type and with content', function () {
      const wrap = render(<StatsGroup>Hello World</StatsGroup>)

      expect(wrap[0].type).toEqual('tag')
      expect(wrap[0].name).toEqual('div')
      // contain styles
      expect(wrap[0].attribs.class).toContain('stats_group')
