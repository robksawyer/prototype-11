...
describe('components', () => {
  describe('LightsBasic', () => {
    it('should mount with props', function () {
      const wrap = mount(<LightsBasic>Hello World</LightsBasic>)
    ...
    it('should render as type and with content', function () {
      const wrap = render(<LightsBasic>Hello World</LightsBasic>)

      expect(wrap[0].type).toEqual('tag')
      expect(wrap[0].name).toEqual('div')
      // contain styles
      expect(wrap[0].attribs.class).toContain('lights_basic')
